...
namespace nimble
{
	...
	void GBufferRenderer::initialize(uint16_t width, uint16_t height)
	{
		...
		Shader* shaders[] = { m_gbuffer_vs, m_gbuffer_fs };
		...
		{
			NIMBLE_LOG_ERROR("Failed to load G-Buffer pass shaders");
		}
	...
	void GBufferRenderer::on_window_resized(uint16_t width, uint16_t height)
	{
		...
		m_gbuffer_rt2 = GlobalGraphicsResources::create_texture_2d(RENDER_TARGET_GBUFFER_RT2, width, height, GL_RGB16F, GL_RGB, GL_HALF_FLOAT);
		m_gbuffer_rt2->set_min_filter(GL_LINEAR);

		m_gbuffer_rt3 = GlobalGraphicsResources::create_texture_2d(RENDER_TARGET_GBUFFER_RT3, width, height, GL_RGBA8, GL_RGBA, GL_UNSIGNED_BYTE);
		...
		Texture* render_targets[] = { m_gbuffer_rt0, m_gbuffer_rt1, m_gbuffer_rt2, m_gbuffer_rt3 };
