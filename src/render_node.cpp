...
namespace nimble
{
	...
	RenderNode::RenderNode(RenderGraph* graph) : m_enabled(true), m_graph(graph), m_id(g_last_node_id++), m_total_time_cpu(0.0f), m_total_time_gpu(0.0f)
	{
	...
	void RenderNode::attach_sub_pass(const std::string& node_name, std::function<void(void)> function)
	{
	...
	void RenderNode::timing_sub_pass(const uint32_t& index, float& cpu_time, float& gpu_time)
	{
