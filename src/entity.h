...
namespace nimble
{
	struct Entity
	{
		...
		inline bool visibility(const uint32_t& view_index) { return (visibility_flags & BIT_FLAG(view_index)) == BIT_FLAG(view_index); }
...
#ifdef ENABLE_SUBMESH_CULLING
		inline bool submesh_visibility(const uint32_t& submesh_index, const uint32_t& view_index) { return (submesh_visibility_flags[submesh_index] & BIT_FLAG(view_index)) == BIT_FLAG(view_index); }
