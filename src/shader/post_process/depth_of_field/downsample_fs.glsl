...
void main()
{
	vec2 tex_coord_00 = PS_IN_TexCoord + vec2(-0.25f, -0.25f) * u_PixelSize;
	vec2 tex_coord_10 = PS_IN_TexCoord + vec2( 0.25f, -0.25f) * u_PixelSize;
	vec2 tex_coord_01 = PS_IN_TexCoord + vec2(-0.25f,  0.25f) * u_PixelSize;
	vec2 tex_coord_11 = PS_IN_TexCoord + vec2( 0.25f,  0.25f) * u_PixelSize;

	vec3 color = texture(s_Color, PS_IN_TexCoord).xyz;
	...
	PS_OUT_Color = color;
	PS_OUT_MulCoCFar = color_mul_coc_far;
	PS_OUT_CoC = coc;
}
