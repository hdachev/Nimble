...
void main()
{
    vec3 result = texture(s_Color, PS_IN_TexCoord).xyz;

	// Far Field
	{
		vec2 tex_coord_00 = PS_IN_TexCoord;
		vec2 tex_coord_10 = PS_IN_TexCoord + vec2(u_PixelSize.x, 0.0);
		vec2 tex_coord_01 = PS_IN_TexCoord + vec2(0.0, u_PixelSize.y);
		vec2 tex_coord_11 = PS_IN_TexCoord + vec2(u_PixelSize.x, u_PixelSize.y);

		float coc_far = texture(s_CoC, PS_IN_TexCoord).y;
		...
		vec2 image_coord = PS_IN_TexCoord / u_PixelSize;
