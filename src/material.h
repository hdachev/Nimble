...
public:
    ...
    {
        m_name = name;
        m_hash = NIMBLE_HASH(m_name.c_str());
    }
    ...
    inline void set_vertex_shader_func(const std::string& src)
    {
        if (src.size() > 0)
        {
            m_vertex_shader_func     = src;
            m_has_vertex_shader_func = true;
        }
    }
    inline void set_fragment_shader_func(const std::string& src)
    {
        if (src.size() > 0)
        {
            m_fragment_shader_func     = src;
            m_has_fragment_shader_func = true;
        }
    }
    inline void set_custom_texture_count(const uint32_t& count) { m_custom_texture_count = count; }
    inline void set_custom_texture(const uint32_t& index, std::shared_ptr<Texture> texture)
    {
        if (index < m_custom_texture_count) { m_custom_textures[index] = texture; }
    }
