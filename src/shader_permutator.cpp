...
namespace nimble
{
    ...
    Program* ShaderPermutator::lookup(const uint64_t& key)
    {
    ...
    Program* ShaderPermutator::compile(const uint64_t& key)
    {
        uint32_t pos = 0;
        ...
        Shader* vs = compile_shader(GL_VERTEX_SHADER, m_vs_src, defines);
        Shader* fs = compile_shader(GL_FRAGMENT_SHADER, m_fs_src, defines);
    ...
    Shader* ShaderPermutator::compile_shader(GLenum type, std::string src, std::vector<std::string>& defines)
    {
