...
namespace nimble
{
	...
	void Bloom::initialize(uint16_t width, uint16_t height)
	{
...

		{
			...
			Shader* shaders[] = { m_quad_vs, m_bright_pass_fs };
			...
			{
				NIMBLE_LOG_ERROR("Failed to load Bloom bright pass shaders");
			}
		}

		{
			...
			Shader* shaders[] = { m_quad_vs, m_bloom_downsample_fs };
			std::string combined_path = vs_path + fs_path;
			...
			{
				NIMBLE_LOG_ERROR("Failed to load Bloom downsample pass shaders");
			}
		}

		{
			...
			Shader* shaders[] = { m_quad_vs, m_bloom_upsample_fs };
			std::string combined_path = vs_path + fs_path;
			...
			{
				NIMBLE_LOG_ERROR("Failed to load Bloom Upsample pass shaders");
			}
		}

		{
			...
			Shader* shaders[] = { m_quad_vs, m_bloom_composite_fs };
			std::string combined_path = vs_path + fs_path;
			...
			{
				NIMBLE_LOG_ERROR("Failed to load Bloom blur pass shaders");
			}
		}
	}
