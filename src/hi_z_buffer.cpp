...
void HiZBuffer::initialize(uint16_t width, uint16_t height)
{
...

	{
		...
		dw::Shader* shaders[] = { m_quad_vs, m_hiz_fs };
		...
		{
			DW_LOG_ERROR("Failed to load Hi-Z shaders");
		}
	}
	...
	// Copy
	{
		...
		dw::Shader* shaders[] = { m_quad_vs, m_copy_fs };
		...
		{
			DW_LOG_ERROR("Failed to load Copy shaders");
		}
	}
}
