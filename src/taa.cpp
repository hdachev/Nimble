...
namespace nimble
{
	...
	void TAA::initialize(uint16_t width, uint16_t height)
	{
...

		{
			...
			Shader* shaders[] = { m_quad_vs, m_taa_fs };
			...
			{
				NIMBLE_LOG_ERROR("Failed to load TAA shaders");
			}
		}

		{
			...
			Shader* shaders[] = { m_quad_vs, m_taa_hist_fs };
			std::string combined_path = vs_path + fs_path;
			...
			{
				NIMBLE_LOG_ERROR("Failed to load TAA History shaders");
			}
		}
