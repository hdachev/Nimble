...
{
    NIMBLE_SAFE_DELETE(m_delta_irradiance_texture);
    NIMBLE_SAFE_DELETE(m_delta_rayleigh_scattering_texture);
    NIMBLE_SAFE_DELETE(m_delta_mie_scattering_texture);
    NIMBLE_SAFE_DELETE(m_delta_scattering_density_texture);
    NIMBLE_SAFE_DELETE(m_transmittance_array[0]);
    NIMBLE_SAFE_DELETE(m_transmittance_array[1]);
    NIMBLE_SAFE_DELETE(m_irradiance_array[0]);
    NIMBLE_SAFE_DELETE(m_irradiance_array[1]);
    NIMBLE_SAFE_DELETE(m_scattering_array[0]);
    NIMBLE_SAFE_DELETE(m_scattering_array[1]);
    NIMBLE_SAFE_DELETE(m_optional_single_mie_scattering_array[0]);
    NIMBLE_SAFE_DELETE(m_optional_single_mie_scattering_array[1]);
}

// -----------------------------------------------------------------------------------------------------------------------------------

void TextureBuffer::clear(Program* program_2d, Program* program_3d)
{
...
void TextureBuffer::new_texture_3d_array(Texture** arr, int width, int height, int depth, bool half_precision)
{
...
Texture* TextureBuffer::new_texture_2d(int width, int height, bool half_precision)
{
    Texture* texture = new Texture2D(width, height, 1, 1, 1, half_precision ? GL_RGBA16F : GL_RGBA32F, GL_RGBA, half_precision ? GL_HALF_FLOAT : GL_FLOAT);
...
Texture* TextureBuffer::new_texture_3d(int width, int height, int depth, bool half_precision)
{
    Texture* texture = new Texture3D(width, height, depth, 1, half_precision ? GL_RGBA16F : GL_RGBA32F, GL_RGBA, half_precision ? GL_HALF_FLOAT : GL_FLOAT);
    texture->set_min_filter(GL_LINEAR);
    texture->set_wrapping(GL_CLAMP_TO_EDGE, GL_CLAMP_TO_EDGE, GL_CLAMP_TO_EDGE);
...
void TextureBuffer::clear_texture(Program* program, Texture* tex)
{
    ...
    {
        Texture3D* tex_3d = (Texture3D*)tex;
    ...
    {
        Texture2D* tex_2d = (Texture2D*)tex;
