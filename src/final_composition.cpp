...
namespace nimble
{
	...
	FinalComposition::FinalComposition()
	{
		...
		Shader* shaders[] = { m_composition_vs, m_composition_fs };
		...
		{
			NIMBLE_LOG_INFO("Failed to load Composition pass shaders");
		}
	...
	void FinalComposition::render(Camera* camera, uint32_t w, uint32_t h)
	{
		...
		if (m_composition_program->set_uniform("s_BrightPass", 11))
			GlobalGraphicsResources::lookup_texture(RENDER_TARGET_SSAO)->bind(11);
