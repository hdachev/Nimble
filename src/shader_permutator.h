...
public:
    ...
    dw::Program* lookup(const uint64_t& key);
    
private:
    int find_bits_required(int combinations);
    dw::Program* compile(const uint64_t& key);
    dw::Shader* compile_shader(GLenum type, std::string src, std::vector<std::string>& defines);
...
private:
    ...
    std::unordered_map<uint64_t, dw::Program*> m_program_cache;
};
