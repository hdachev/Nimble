...
namespace nimble
{
	...
	void Renderer::initialize(uint16_t width, uint16_t height, Camera* camera)
	{
		m_width = width;
		m_height = height;
		set_camera(camera);
		...
		//	{
		//		NIMBLE_LOG_ERROR("Failed to load cubemap shaders");
		//	}
		//}
		...
		//	{
		//		NIMBLE_LOG_ERROR("Failed to load PSSM shaders");
		//	}
		//}
	...
	void Renderer::update_uniforms(Camera* camera, double delta)
	{
	...
	void Renderer::render(double delta)
	{
		...
		if (!m_scene)
		{
			NIMBLE_LOG_ERROR("Scene has not been set!");
			return;
		}
